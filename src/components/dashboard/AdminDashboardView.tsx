...
import { Batch, StorageUnit, AppUser, FeedbackRecord, StakeholderRole } from '../../types';
...
export const AdminDashboardView: React.FC<AdminDashboardViewProps> = ({ user, onSelectBatch }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const allF = await traceService.getAllFeedbacks();
      setBatches(allB);
  ...
  return (
    <div className="space-y-8 pb-16 animate-fadeIn">
      ...
      {/* Platform Feedback & Stakeholder Reputation Stream */}
      <div className="bg-white rounded-3xl border border-slate-200/90 p-6 md:p-8 space-y-6 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-bold text-slate-900">Network Feedback & Stakeholder Ratings</h2>
            </div>
            <p className="text-xs text-slate-500">
              Live audit stream of all two-way evaluations stored in Supabase `feedbacks` and `audit_logs` tables.
            </p>
          </div>

          <button
            onClick={() => setIsFeedbackModalOpen(true)}
            className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-2xl text-xs transition-all shadow-xs flex items-center gap-1.5 self-start sm:self-auto cursor-pointer"
          >
            <Plus className="w-4 h-4" />
            <span>Submit Administrative Feedback</span>
          </button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 pb-2 border-b border-slate-100">
          ...
          {['ALL', 'FARMER', 'MANDI', 'WAREHOUSE', 'PROCESSOR', 'TRANSPORTER', 'RETAILER', 'CONSUMER'].map((r) => (
            <button
              key={r}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {feedbacks
            .filter((f) => roleFilter === 'ALL' || f.fromRole === roleFilter || f.toRole === roleFilter)
            .map((fb) => (
              <div
                key={fb.feedbackId}
                className="p-5 rounded-2xl bg-slate-50 border border-slate-200/80 hover:bg-white hover:border-emerald-400 hover:shadow-xs transition-all space-y-2.5 flex flex-col justify-between"
              >
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-sm text-slate-900">{fb.submittedBy}</span>
                      <span className="px-2 py-0.5 rounded-full bg-slate-200/80 text-slate-800 text-[10px] font-mono font-bold uppercase">
                        {fb.fromRole} → {fb.toRole}
                      </span>
                    </div>
                  ...
                  <div className="text-[11px] text-slate-500 font-mono">
                    Batch: <span className="font-bold text-slate-800">{fb.batchId}</span> · Category: {fb.category}
                  </div>
      ...
      {isFeedbackModalOpen && (
        <UnifiedFeedbackModal
          ...
          initialBatchId={batches[0]?.batchId || 'BIS-2026-092'}
          fromRole="ADMIN"
          targetRole="FARMER"
          targetEntityName="Network Audit Participant"
